...
func beginTx(databaseName string, isolationLevel *IsolationLevel, maxTxTimeoutSec *int) (txId string, nodeIdx int, err error) {
	...
	resp, err := executor.Request(true, databaseName, ep_TX_BEGIN, http.MethodPost, headers, body, 0, 2)
	if err != nil {
		return "", -1, err
	}
	defer resp.Release()
...
func (c *TxClient) Query(sql string, params *Params, opts *QueryOptions) (*Records, error) {
	...
	resp, err := c.executor.RequestTx(c.nodeIdx, ep_TX_QUERY, http.MethodPost, headers, body, timeoutSec, 2)
	if err != nil {
		return nil, err
	}
	defer resp.Release()
...
func (c *TxClient) Execute(sql string, params *Params) (*ExecuteResult, error) {
	...
	resp, err := c.executor.RequestTx(c.nodeIdx, ep_TX_EXECUTE, http.MethodPost, headers, body, 0, 2)
	if err != nil {
		return nil, err
	}
	defer resp.Release()
...
func (c *TxClient) Commit() error {
	...
	resp, err := c.executor.RequestTx(c.nodeIdx, ep_TX_COMMIT, http.MethodPut, headers, nil, 0, 2)
	if err != nil {
		return err
	}
	defer resp.Release()
...
func (c *TxClient) Rollback() error {
	...
	resp, err := c.executor.RequestTx(c.nodeIdx, ep_TX_ROLLBACK, http.MethodPut, headers, nil, 0, 2)
	if err != nil {
		return err
	}
	defer resp.Release()
...
func (c *TxClient) Close() error {
	...
	resp, err := c.executor.RequestTx(c.nodeIdx, ep_TX_CLOSE, http.MethodPut, headers, nil, 0, 3)
	if err != nil {
		return err
	}
	defer resp.Release()
